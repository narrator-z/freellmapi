...
import { reapplyCachedAugmentedCatalog, syncAugmentedCatalog } from './augmented-catalog-sync.js';
...
export function startCatalogSync(scheduler: Scheduler): void {
  ...
  const run = () => {
    ...
    // Fork: prefer the self-hosted augmented catalog as the data source so
    // the install keeps syncing the fork's own model list (URL configurable
    // via AUGMENTED_CATALOG_URL). Upstream's signed syncCatalog() remains
    // exported and is available as a fallback / for premium tiers.
