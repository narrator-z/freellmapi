...
function isAutoModel(modelId: string | undefined): boolean {
  ...
  // #fork: `freellmauto` is an exact alias of `auto` for tools that can't send
  // the bare `auto` id, so normalize before the auto checks.
...
proxyRouter.get('/models', (req: Request, res: Response) => {
  ...
  res.json({
    ...
    data: [
      ...
      {
        // #fork: `freellmauto` is an exact alias of `auto` for clients/tools that
        // can't send the bare `auto` id. Same routing behavior and ceiling.
        id: 'freellmauto',
        object: 'model',
        created: 0,
        owned_by: 'freellmapi',
