          ...
          {messages.length === 0 ? (
            <div className="flex items-center justify-center h-full text-center">
              <div className="space-y-2 max-w-sm">
                <p className="text-base font-medium">{t('playground.sendMessage')}</p>
                <p className="text-sm text-muted-foreground">
                  {t('playground.usingModel')} <span className="text-foreground">{activeModelLabel}</span>. {t('playground.switchModels')}
                </p>
              </div>
            </div>
          ) : (
